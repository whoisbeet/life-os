...
import { fmtDate, smartDate, dateColor, calculateCurrentStreak, bangkokDateKey } from "@/lib/dates";
...
function HabitSection({ itemId, logs, meta, accentColor }: { itemId: string; logs: any[]; meta: any; accentColor: string }) {
  ...
  const doneCount = days.filter((d) => logSet.has(bangkokDateKey(d))).length;
