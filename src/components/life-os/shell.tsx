...
function MobileBottomNav({ view, onNavigate }: { view: string; onNavigate: (view: any) => void }) {
  return <nav aria-label="Primary navigation" className="fixed inset-x-0 bottom-0 z-40 border-t border-border/60 bg-background/90 px-2 pt-1 shadow-lg backdrop-blur-md md:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}><div className="mx-auto flex max-w-lg items-center justify-around">{MOBILE_NAV.map((item) => { const active = view === item.key; return <button key={item.key} onClick={() => onNavigate(item.key)} aria-current={active ? "page" : undefined} className={`flex min-w-0 flex-1 flex-col items-center gap-0.5 rounded-lg py-2 text-[10px] font-medium transition-colors ${active ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}><Icon name={item.icon} className="h-5 w-5" /><span>{item.label}</span></button>; })}</div></nav>;
}
